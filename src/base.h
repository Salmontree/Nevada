...
#define VecPush(vector, ...) __base_vec_push((void **)&(vector).data, &(vector).length, &(vector).capacity, sizeof(*(vector).data), &(__VA_ARGS__));
...
#define VecUnshift(vector, ...) __base_vec_unshift((void **)&(vector).data, &(vector).length, &(vector).capacity, sizeof(*(vector).data), &(__VA_ARGS__))
void __base_vec_unshift(void **data, size_t *length, size_t *capacity, size_t element_size, const void *value);

#define VecInsert(vector, index, ...) __base_vec_insert((void **)&(vector).data, &(vector).length, &(vector).capacity, sizeof(*(vector).data), &(__VA_ARGS__), (index))
...
String StrSlice(Arena *arena, String str, ssize_t start, ssize_t end);
bool StrIncludes(String source, String subStr);
ssize_t StrFind(String source, String subStr);
ssize_t StrFindFrom(String source, String subStr, size_t index);
ssize_t StrFindBack(String source, String subStr);
...
String StrSlice(Arena *arena, String str, ssize_t start, ssize_t end) {
  ...
  Assert(start <= (ssize_t)str.length, "StrSlice: start index out of bounds");
...
ssize_t StrFind(String source, String sub_str) {
  Assert(!StrIsNull(source), "StrFind: source should never be NULL");
  Assert(!StrIsNull(sub_str), "StrFind: sub_str should never be NULL");
...
ssize_t StrFindFrom(String source, String sub_str, size_t index) {
  Assert(!StrIsNull(source), "StrFind: source should never be NULL");
  Assert(!StrIsNull(sub_str), "StrFind: sub_str should never be NULL");

  if (source.length == 0 || sub_str.length == 0 || sub_str.length > source.length || index > source.length - sub_str.length) {
    return -1;
  }

  for (size_t i = index; i <= source.length - sub_str.length; i++) {
    if (source.data[i] != sub_str.data[0]) {
      continue;
    }
...
ssize_t StrFindBack(String source, String sub_str) {
  Assert(!StrIsNull(source), "StrFindFromBack: source should never be NULL");
  Assert(!StrIsNull(sub_str), "StrFindFromBack: sub_str should never be NULL");

  if (source.length == 0 || sub_str.length == 0 || sub_str.length > source.length) {
    return -1;
  }

  for (size_t i = source.length - sub_str.length; i >= 0; i--) {
    if (source.data[i] != sub_str.data[0]) {
      continue;
    }
